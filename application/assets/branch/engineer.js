$(document).ready(function() {
    ...
    function show_data(){
        $.ajax({
            method :'GET',
            url  : api_base_url+'branch/engineer/list_engineers',
            dataType : 'json',
            async: false,       
            processData: false,
            contentType: false,   
            ...
            success : function(data){
                ...
                for(i=0; i<data.length; i++){
                    ...
                    '<td class="item_view"  data-user_id="'+data[i].user_id+'">'+data[i].branch_name+'</td>'+
    ...
    $("#form_add").validate({
        ...
        submitHandler: function(form) { add_engineer_users(); }
    });


    function add_engineer_users() {

        var adddata = new FormData(document.getElementById("form_add"));
        $.ajax({
            type : "POST",
            url  : api_base_url+'branch/engineer/create',
            dataType : "JSON",
            data : adddata,
            async: false,
            processData: false,
            ...
            success: function(data){
                ...
                console.log(data);
                window.location.href = base_url+"branch/engineer";
    ...
    $('#example').on('click','.item_view',function(){
        ...
        window.location.href = base_url+"branch/engineer/edit/"+user_id;

    });


    // delete engineer_users - start
    ...
    $('#delete_bt').on('click',function(){
        ...
        $.ajax({
            type : "POST",
            url  : api_base_url+'branch/engineer/delete',
            dataType : "JSON",
    ...
    $("#form_update").validate({
        ...
        submitHandler: function(form) { update_engineer_user(); }
    });


    function update_engineer_user() {

        var adddata = new FormData(document.getElementById("form_update"));
        $.ajax({
            type : "POST",
            url  : api_base_url+'branch/engineer/update',
            dataType : "JSON",
            data : adddata,
            async: false,
            processData: false,
...
function getDropdownData(){
    $.ajax({
        method :'GET',
        url  : api_base_url+'branch/engineer/getData',
        dataType : 'json', 
        headers: {
            "auth-key":"simplerestapi",
            'client-service' : "frontend-client",
        ...
        success: function(data){
            ...
            if($('#select_branch').length > 0){
                ...
                $('#select_branch').empty();
                var branchResult = '';
                document.getElementById('select_branch').innerHTML='<option value="0" selected disabled>Select Branch</option>';
...
function getUserData(user_id){
    $.ajax({
        method :'POST',
        url  : api_base_url+'branch/engineer/detail',
        data:{'user_id':user_id},
        dataType : 'json', 
        headers: {
            "auth-key":"simplerestapi",
        ...
        success: function(res){
            ...
            foreman_list = res.list_foreman;
            $('#foreman').empty();
            var foremanResult = '';
            for(i=0; i<foreman_list.length; i++){
                if(foreman_list[i].assign_to == user_id){
                    var selected = 'selected = "selected"';
                }else{
                    var selected = '';
                }
                foremanResult += "<option "+selected+" value='"+foreman_list[i].user_id+"'>"+foreman_list[i].fname+' '+foreman_list[i].lname+"</option>";
            }
            $( '#foreman' ).append( foremanResult ); 

            var select = document.getElementById("foreman");
            multi(select, {
                non_selected_header: "Foremen",
                selected_header: "Selected Foremen"
            });

            list_tech = res.list_tech;
            $('#technician').empty();
            var technicianResult = '';
            for(i=0; i<list_tech.length; i++){
                if(list_tech[i].assign_to == user_id){
                    var selected = 'selected = "selected"';
                }else{
                    var selected = '';
                }
                technicianResult += "<option "+selected+" value='"+list_tech[i].user_id+"'>"+list_tech[i].fname+' '+list_tech[i].lname+"</option>";
            }
            console.log(technicianResult)
            $( '#technician' ).append( technicianResult ); 

            var select = document.getElementById("technician");
            multi(select, {
                non_selected_header: "Technician ",
                selected_header: "Selected Technician "
            });

            list_techassistant = res.list_techassistant;
            $('#technician_assistant').empty();
            var technician_assistantResult = '';
            for(i=0; i<list_techassistant.length; i++){
                if(list_techassistant[i].assign_to == user_id){
                    var selected = 'selected = "selected"';
                }else{
                    var selected = '';
                }
                technician_assistantResult += "<option "+selected+"  value='"+list_techassistant[i].user_id+"'>"+list_techassistant[i].fname+' '+list_tech[i].lname+"</option>";
            }
            $( '#technician_assistant' ).append( technician_assistantResult ); 

            var select = document.getElementById("technician_assistant");
            multi(select, {
