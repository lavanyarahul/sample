$(document).ready(function() {
    ...
    function show_data(){
        $.ajax({
            method :'GET',
            url  : api_base_url+'admin/checklist/list_checklist',
            dataType : 'json',
            async: false,
            processData: false,
            contentType: false,
            ...
            success : function(data){
                ...
                for(i=0; i<ride_data.length; i++){
                    ...
                    html += '<tr>'+
                    '<td class="item_view " data-checklist_id="'+ride_data[i].checklist_id+'" data-track_id="'+ride_data[i].track_id+'">'+sl_no+'</td>'+
                    '<td class="item_view " data-checklist_id="'+ride_data[i].checklist_id+'" data-track_id="'+ride_data[i].track_id+'">'+ride_data[i].checklist_name+'</td>'+
                     '<td class="item_view " data-checklist_id="'+ride_data[i].checklist_id+'" data-track_id="'+ride_data[i].track_id+'">'+ride_data[i].fname+' '+ride_data[i].lname+'</td>'+
                    '<td class="item_view " data-checklist_id="'+ride_data[i].checklist_id+'" data-track_id="'+ride_data[i].track_id+'">'+ride_data[i].created_datetime+'</td>'+
                    '<td >'+ status+'</td>'+
                    '</tr>';
                }
                ...
                for(i=0; i<pool_data.length; i++){
                    ...
                    pool_html += '<tr>'+
                    '<td class="item_view " data-checklist_id="'+pool_data[i].checklist_id+'" data-track_id="'+pool_data[i].track_id+'">'+sl_no+'</td>'+
                    '<td class="item_view " data-checklist_id="'+pool_data[i].checklist_id+'" data-track_id="'+pool_data[i].track_id+'">'+pool_data[i].checklist_name+'</td>'+
                     '<td class="item_view " data-checklist_id="'+pool_data[i].checklist_id+'" data-track_id="'+pool_data[i].track_id+'">'+pool_data[i].fname+' '+pool_data[i].lname+'</td>'+
                    '<td class="item_view " data-checklist_id="'+pool_data[i].checklist_id+'" data-track_id="'+pool_data[i].track_id+'">'+pool_data[i].created_datetime+'</td>'+
                    '<td >'+ status+'</td>'+
                    '</tr>';
                }
    ...
    $('#rideTbl').on('click','.item_view',function(){
        ...
        window.location.href = base_url+"admin/checklist/view/"+checklist_id+'/'+track_id;
...
function getChecklistData(checklist_id,track_id){
    $.ajax({
        method :'POST',
        url  : api_base_url+'admin/checklist/get_checklist',
            ...
            if(checklist.checklist_id != '') {
                    ...
                    for(i=0; i<criteria.length; i++){
                        ...
                        if(criteria[i].criteria_type == 1 ) {
                            if(criteria[i].criteria_value == 1){ 
                                criteriaBody += '<td class="borderx text-success "><i class="fa fa-check" aria-hidden="true"></i></td>'
                            }else{
                                criteriaBody += '<td class="borderx text-danger "> <i class="fa fa-times" aria-hidden="true"></i></td>'    }
                ...
                if(breakdown.length > 0){
                    ...
                    for(i=0; i<breakdown.length; i++){
                        ...
                        html += '<tr>'+
                        '<td class="" data-checklist_id="'+breakdown[i].checklist_id+'">'+sl_no+'</td>'+
                        '<td class="" data-checklist_id="'+breakdown[i].checklist_id+'"><div class="widget-content p-0"> <div class="widget-content-wrapper"> <div class="widget-content-left flex2"> <div class="widget-heading">'+breakdown[i].ride_name+'</div>'+
                        '  </div> </div> </div></td>'+
                        '<td class="">'+breakdown[i].complaint+'</td>'+  
                        '<td class="">'+breakdown[i].comment+'</td>'+  
                        '<td class="">'+breakdown[i].fname+' '+breakdown[i].lname+'</td>'+  
                        '<td class="">'+breakdown[i].created_datetime+'</td>'+       
                        '<td >'+ status+'</td>'+
                        '</tr>';
                    }
                    $('#tbl_data').html(html);
