$(document).ready(function() {  
    ...
    $('#example').on('click','.item_reassign',function(){
        ...
        $.ajax({
            method :'GET',
            url  : api_base_url+'admin/complaint/getTech',
            dataType : 'json', 
            headers: {
    ...
    function reassign_comp(){
        ...
        $.ajax({
            type : "POST",
            url  : api_base_url+'admin/complaint/reassign',
            dataType : "JSON",
            data : adddata,
            async: false,
            processData: false,
    ...
    $('#close_bt').click( function(){
       ...
       var complaint_id = $('#comp_id').val();   
        ...
        $.ajax({
            type : "POST",
            url  : api_base_url+'admin/complaint/close',
            dataType : "JSON",
    ...
    function assign_comp(){
        ...
        $.ajax({
            type : "POST",
            url  : api_base_url+'admin/complaint/assign',
            dataType : "JSON",
            data : adddata,
            async: false,
            processData: false,
...
function show_data(){
    $.ajax({
        method :'GET',
        url : api_base_url+'admin/complaint/getList',  
        dataType : 'json',
        async: false,       
        processData: false,
        contentType: false,   
        ...
        success : function(data){
            if(data.status == 401){
                ...
                for(i=0; i<data.length; i++){
                    ...
                    if(data[i].status == 1){
                        var status =  '';
                        var btn =  '<a style="color:white;" class="btn btn-warning btn-sm item_assign" data-toggle="modal" data-target="#assign_modal" data-complaint_id="'+data[i].complaint_id+'" title="Assign"><i class="ti-trash mr-0-5"></i>Assign</a>'; 
                        ...
                        var status = '';  
                        var btn = '<a style="color:white;" class="btn btn-success btn-sm item_close" data-toggle="modal" data-target="#close_modal" data-complaint_id="'+data[i].complaint_id+'" title="Close"><i class="ti-trash mr-0-5"></i>Close</a>'; 
                    }else {
                        var status = 'Closed';  
                        var btn = ''; 
                     }
